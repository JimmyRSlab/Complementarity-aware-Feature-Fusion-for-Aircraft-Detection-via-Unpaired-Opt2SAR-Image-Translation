...
class LoadImagePairFromFile(LoadImageFromFile):
    ...
    Args:
        modalities (tuple/list): Names of folders denoting different modalities.
        to_float32 (bool): Whether to convert the loaded image to a float32
            numpy array. If set to False, the loaded image is an uint8 array.
            Defaults to False.
        color_type (str): The flag argument for :func:`mmcv.imfrombytes`.
    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            filename1 = osp.join(results['img_prefix'], self.modalities[0],
                                results['img_info']['filename'])
            filename2 = osp.join(results['img_prefix'], self.modalities[1],
                                results['img_info']['filename'])
        else:
            filename1 = osp.join(self.modalities[0], results['img_info']['filename'])
            filename2 = osp.join(self.modalities[1], results['img_info']['filename'])
